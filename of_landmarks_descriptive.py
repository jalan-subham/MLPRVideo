...
class FacialLandmarksOpenFace(Dataset):
    ...
    def __init__(self, paths, transform=None):
        ...
        self.start_col = np.where(cols == " gaze_angle_x")[0][0]
        self.end_col = np.where(cols == " gaze_angle_y")[0][0]
    ...
    def load_expressions(self, file_path):
        ...
        # np.save( "/".join(file_path.split('/')[:-1]) + "/of_landmarks_pro.npy", df)
        df = np.load( "/".join(file_path.split('/')[:-1]) + "/of_landmarks_pro.npy")
...
with open("OFLandmarksDescriptiveGaze-SVM.txt", "a") as file:
