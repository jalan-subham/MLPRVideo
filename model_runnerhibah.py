...
for j in range(5):
    ...
    for i in tqdm.tqdm(range(user_count)):
        ...
        os.system(f"python3 of_landmarks_descriptive_gaze_rf.py {i}")
    with open("OFLandmarksDescriptiveGaze-RF.txt", "r") as file:
        accs = [float(x.strip()) for x in file.readlines()
                ]
        accs_.append(sum(accs)/len(accs))
    print(accs_)
    os.remove("OFLandmarksDescriptiveGaze-RF.txt")
