...
model = LandmarkLSTM(7, 1, 2).to(device)
...
plt.savefig('outputs/Expressions_LSTM_' + str(i))
with open("expressions_pt_loo.txt", "a") as file:
    file.write(f"{i}, {np.array(model_results['test_acc']).mean()}")